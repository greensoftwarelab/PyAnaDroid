...
from textops import grep, cat, sed, head
import json
import re
    ...
    def needs_min_sdk_upgrade(self, gradle_file):
        ...
        if str(has_min_sdk) != "":
            ...
            if int(str(min_sdk)) > device_sdk_version:
                log(
                    f"This app target sdk version {min_sdk}. This is greater than the device version and the application"
    ...
    def __set_build_tools_version(self, bld_file, btools_version=DEFAULT_BUILD_TOOLS_VERSION):
        ...
        if has_bld_tools != "":
            if can_be_semantic_version(has_bld_tools):
                ...
                x = list(filter(
                    lambda t: re.search(target_pattern1, str(cat(t))) and re.search(target_pattern2, str(cat(t))),
                    mega_find(self.proj.proj_dir, "*.gradle", type_file='f')))
                if len(x) > 0:
                    # assume 0
                    pattern = re.sub(r'\"|\'', "",
                                     re.search(target_pattern2, str(cat(x[0]))).group().split("=")[1].strip())
